...
void AEC_Displacement_UpdateByVelocity(AEC_EntityCatalog* entityCatalog, unsigned int entity_at, float step)
{
    if (entityCatalog->entity_id[entity_at] > 0)
    {
        ...
        {
            if (entityCatalog->velocity[entity_at].xVel != 0 && entityCatalog->velocity[entity_at].yVel != 0)
            {
                ...
                if (current_x < new_x)
                {
                    ...
                    AEC_CharacterSprite_SetFlip(entityCatalog, entity_at, SDL_FLIP_HORIZONTAL);
                }
                else if (current_x > new_x)
                {
                    entityCatalog->displacement[entity_at].reflection = SDL_FLIP_NONE;
                    AEC_CharacterSprite_SetFlip(entityCatalog, entity_at, SDL_FLIP_NONE);
                }
            }
